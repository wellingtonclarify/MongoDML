...
namespace MongoDML.Core
{
    public class Cerca : BaseDbEntity
    {
        ...
        public static List<Cerca> GetAll()
        {
            ...
            {
                var query = @"select c.Id, c.Descricao, t.Tipo, c.LatitudeGaragem, c.LongitudeGaragem from CercaEletronica c join CercaEletronicaTipo t on t.Id = c.TipoId where t.id in (2,3,4) and c.ativo = 1 and c.DtDelete is null order by c.Descricao";
